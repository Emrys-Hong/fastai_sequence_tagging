...
def get_rnn_classifier_Emrys(bptt, max_seq, n_class, n_tok, emb_sz, n_hid, n_layers, pad_token, layers, drops, bidir=False,
                      dropouth=0.3, dropouti=0.5, dropoute=0.1, wdrop=0.5, qrnn=False):
    rnn_enc = MultiBatchRNN(70, 20*70, n_tok, emb_sz, n_hid, n_layers, pad_token=pad_token, bidir=bidir,
                      dropouth=dropouth, dropouti=dropouti, dropoute=dropoute, wdrop=wdrop, qrnn=qrnn)
    crf = to_gpu(CRF(2,1,n_class,n_class))
    return SequentialRNN(rnn_enc, PoolingLinearClassifier(layers, drops)), crf


def train(path, bptt=70, em_sz=400, nh=1150, nl=3, ):
    """import data"""
    dir_path = Path(path)
    bs=64 # batch_size = 2
    ...
    m, crf = get_rnn_classifier_seq(bptt, 20*70, c, vs, emb_sz=em_sz, 
